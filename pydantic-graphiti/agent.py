...
graphiti_agent = Agent(
    get_model(),
    system_prompt="""You are a helpful assistant with access to a knowledge graph filled with temporal data about MCU heroes.
    When the user asks you a question, use your search tool to query the knowledge graph and then answer honestly.
    The data shows the evolution of heroes before and after Endgame events.
...
async def suggest_questions():
    """Suggest some interesting questions to ask about MCU heroes."""
    questions = [
        "Is Tony Stark alive?",
        "Who is Captain America?", 
        "What happened to Natasha Romanoff?",
        "Where is Thor now?",
        "What is Bruce Banner's current state?",
        "Who leads the Avengers now?",
        "What happened to Steve Rogers?"
    ]
    
    print("\n" + "="*50)
    print("🦸 SUGGESTED MCU QUESTIONS 🦸")
    print("="*50)
    for i, question in enumerate(questions, 1):
        print(f"{i}. {question}")
    print("="*50)
    print("Type any of these questions or ask your own!")
    print("="*50)
...
async def main():
    ...
    print("🦸 MCU Heroes Knowledge Graph - Powered by Pydantic AI & Graphiti 🦸")
